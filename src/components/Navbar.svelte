<script>
    ...
    function navScroller() {
        window.$(document).scroll(function () {
            window.$('.rvp-nav-bg').toggleClass("rvp-nav-alt", (window.$(this).scrollTop() > 50));
            window.$('.rvp-toggler').toggleClass("rvp-toggler-alt", (window.$(this).scrollTop() > 50));
            window.$('.rvp-nav-bg .rvp-links').toggleClass("rvp-font-alt", (window.$(this).scrollTop() > 50));
        });
    };
...
<Router url="{url}">
    <nav class="{ navToggler ? 'navbar sticky-top navbar-expand-lg navbar-dark rvp-nav-bg' : 'rvp-nav-alt'}">
        ...
        <button class={navToggler? "navbar-toggler rvp-toggler": "rvp-toggler-alt"} type="button" id="id-button" on:click={openModal}>
